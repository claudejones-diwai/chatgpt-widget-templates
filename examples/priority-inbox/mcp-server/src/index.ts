// MCP Server for Cloudflare Workers - Priority Inbox
// Serves get_priority_emails tool and returns widget URL

import { handleGetPriorityEmails } from "./tools/get_priority_emails";
import { handleHealth } from "./handlers/health";
import { handleInfo } from "./handlers/info";

// Widget URL - stable production URL (auto-updates with each deployment)
const WIDGET_URL = "https://priority-inbox.pages.dev";
        ...
        try {
          ...
          switch (body.method) {
                ...
                tools: [
                  {
                    ...
                    description:
                      "Retrieves and displays priority emails from inbox with interactive widget. Use this when users ask about their emails, inbox, messages, or want to see important communications. Supports filtering by category (primary, social, promotions, updates) and showing only unread emails.",
                    inputSchema: {
                      type: "object",
                      properties: {
                        category: {
                          ...
                          enum: ["all", "primary", "social", "promotions", "updates"],
                          default: "all",
                        },
                        limit: {
                          type: "number",
            ...
            case "resources/read":
              ...
              // This ensures we always get the correct asset hashes after rebuilds
            ...
            case "tools/call":
              ...
              if (toolName === "get_priority_emails") {
                const emailData = handleGetPriorityEmails(toolArgs);

                result = {
                  content: [
                    {
                      type: "text",
                      text: `Found ${emailData.emails.length} ${emailData.category === "all" ? "" : emailData.category + " "}email${emailData.emails.length !== 1 ? "s" : ""} in your inbox`,
                    },
                  ],
